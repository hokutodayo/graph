...
export class Utils {
	...
	public static confirmAction(message: string, action: () => void) {
		Swal.fire({
			title: "確認",
			text: message,
			icon: "warning",
	...
	// 位置に近い頂点を取得する
	public static findVertexAt(x: number, y: number, vertices: Vertex[]): Vertex | null {
		const foundVertex = vertices.find((vertex) => vertex.isNear(x, y));
		return foundVertex ? foundVertex : null;
	}

	// 位置に近い制御点を取得する
	public static findControlAt(x: number, y: number, edges: Edge[]): Control | null {
		const foundControl = edges.find((edge) => edge.control.isNear(x, y));
		return foundControl ? foundControl.control : null;
	}

	// 位置に近い辺を取得する
	public static findEdgeAt(x: number, y: number, edges: Edge[]): Edge | null {
